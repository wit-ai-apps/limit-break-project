const STORAGE_KEY = "limitBreakProjectRecordsV120";
    ...
    const FALLBACK_MATERIALS_OUTLINE = {
      version: "3.5.0",
    ...
    const FALLBACK_SUBJECT_BALANCE = {
      version: "3.5.0",
    ...
    const FALLBACK_REVERSE_PROGRESS = {
      version: "1.7.0",
    ...
    const FALLBACK_REVIEW_SCHEDULE = {
      version: "1.7.0",
    ...
    const FALLBACK_COUNSELOR_NOTES = {
      version: "2.0.0",
      policy: {
    ...
    const FALLBACK_NOTICE_CONTACTS = {
      recipients: [
    ...
    function renderCountdowns() {
      const sorted = allCountdownTargets()
      ...
      sorted.forEach((exam) => {
        ...
        const compactName = countdownLabel(exam);
        card.innerHTML = `
          <h2>${compactName}</h2>
          <strong>あと${days}日</strong>
          <span>${exam.date_start}${exam.source === "custom" ? " / 短期目標" : ""}</span>
        `;
    ...
    function renderScheduleDrawer() {
      if (!scheduleDrawerCountdowns || !scheduleCalendar) return;
      const targets = allCountdownTargets()
        .sort((a, b) => dateDaysUntil(a.countdown_target) - dateDaysUntil(b.countdown_target));

      scheduleDrawerCountdowns.innerHTML = targets.slice(0, 6).map((exam) => {
        const days = dateDaysUntil(exam.countdown_target);
        return `
      ...
      renderScheduleCalendar(targets);
    }

    function renderScheduleCalendar(targets) {
      ...
      const headings = ["日", "月", "火", "水", "木", "金", "土"].map((day) => `<div class="calendar-day is-heading">${day}</div>`);
      ...
      const days = Array.from({ length: lastDay.getDate() }, (_, index) => {
        ...
        return `<div class="${classes}">${day}${labels.slice(0, 1).map((label) => `<small>${escapeHtml(label)}</small>`).join("")}</div>`;
      });

      scheduleCalendar.innerHTML = [...headings, ...blanks, ...days].join("");
    }
